...
# Import persistence init
from app.services.storage.persistence_init import initialize_persistence
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        console.rule("[bold orange]🗄️ PHASE 3.5: Database Init[/bold orange]")
        app_logger.info("PHASE 3.5/5: Initializing database persistence...")
        persistence_manager = initialize_persistence(config)
        app_logger.info("✓ Database persistence initialized")
        ...
        for model_name in essential_models:
            if model_name in registry_config:
                ...
                load_task = model_manager.load_model(model_name)
        ...
        processor = UnifiedProcessor(model_manager, config=config, registry_config=registry_config)
        
        # Store audit_logger, metrics, and persistence_manager as properties
        processor.audit_logger = audit_logger
        processor.metrics = metrics
        processor.metrics_collector = metrics  # For backward compatibility
        processor.persistence_manager = persistence_manager
        ...
        # Initialize session manager cleanup task
        app_logger.info("Initializing session manager cleanup task...")
        try:
            from app.services.storage.session_manager import SessionManager
            session_manager = SessionManager()
            await session_manager.start_cleanup_task()
            app_logger.info("✓ Session manager cleanup task started")
        except Exception as e:
            app_logger.error(f"Error starting session manager cleanup task: {str(e)}", exc_info=True)
            console.print(f"[yellow]⚠ Warning: Error starting session manager cleanup task: {str(e)}[/yellow]")
    ...
    try:
        ...
        console.print("[bold yellow]Starting shutdown sequence...[/bold yellow]")
        ...
        if hasattr(app.state, "metrics") and app.state.metrics:
            try:
                ...
                console.print("[cyan]Saving performance metrics...[/cyan]")
                app.state.metrics.save_metrics()
                app_logger.info("✓ Performance metrics saved successfully")
                console.print("[green]✓ Performance metrics saved successfully[/green]")
            except Exception as metrics_error:
                error_msg = f"Error saving metrics: {str(metrics_error)}"
                app_logger.error(error_msg, exc_info=True)
                console.print(f"[red]✗ {error_msg}[/red]")
                shutdown_errors.append(("metrics", error_msg))
        ...
        if hasattr(app.state, "route_cache") and app.state.route_cache:
            try:
                ...
                console.print("[cyan]Cleaning up route cache...[/cyan]")
                from app.services.storage.route_cache import RouteCacheManager
                await RouteCacheManager.clear_all()
                app_logger.info("✓ Route cache cleared successfully")
                console.print("[green]✓ Route cache cleared successfully[/green]")
            except Exception as cache_error:
                error_msg = f"Error clearing route cache: {str(cache_error)}"
                app_logger.error(error_msg, exc_info=True)
                console.print(f"[red]✗ {error_msg}[/red]")
                shutdown_errors.append(("route_cache", error_msg))
        
        # Unload models - most important part
        if hasattr(app.state, "model_manager") and app.state.model_manager:
            try:
                app_logger.info("Unloading models...")
                console.print("[cyan]Unloading models...[/cyan]")
                await app.state.model_manager.unload_all_models()
                app_logger.info("✓ Models unloaded successfully")
                console.print("[green]✓ Models unloaded successfully[/green]")
            except Exception as model_error:
                error_msg = f"Error unloading models: {str(model_error)}"
                app_logger.error(error_msg, exc_info=True)
                console.print(f"[red]✗ {error_msg}[/red]")
                shutdown_errors.append(("model_unload", error_msg))
        ...
        if shutdown_errors:
            ...
            error_components = ", ".join([err[0] for err in shutdown_errors])
            app_logger.warning(f"CasaLingua server shutdown completed with {error_count} errors in {shutdown_time:.2f}s: {error_components}")
            console.print(Panel(
                f"[bold yellow]⚠ CasaLingua server shutdown completed with {error_count} errors[/bold yellow]\n"
                f"[yellow]Time taken: {shutdown_time:.2f}s[/yellow]\n"
                f"[red]Problem components: {error_components}[/red]",
                border_style="yellow"
            ))
        else:
            app_logger.info(f"✓ CasaLingua server shutdown completed successfully in {shutdown_time:.2f}s")
            console.print(Panel(
                f"[bold green]✓ CasaLingua server shutdown completed successfully[/bold green]\n"
                f"[green]Time taken: {shutdown_time:.2f}s[/green]",
                border_style="green"
            ))
        ...
        # Enhanced: print exception with rich formatting
        console.print_exception()
        console_logger.error(f"Critical error during shutdown: {str(e)}", exc_info=True)
        console.print(Panel(
            f"[bold red]✗ CRITICAL ERROR DURING SHUTDOWN[/bold red]\n"
            f"{str(e)}",
            border_style="red"
        ))
...
# Include verification API routes
