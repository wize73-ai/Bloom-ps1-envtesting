...
class RagQueryResponse(BaseModel):
    ...
    performance_metrics: Optional[Dict[str, Any]] = Field(None, description="Detailed performance metrics")
    memory_usage: Optional[Dict[str, float]] = Field(None, description="Memory usage statistics")
    operation_cost: Optional[float] = Field(None, description="Estimated cost of operation")
    accuracy_score: Optional[float] = Field(None, description="Accuracy score (0-1)")
    truth_score: Optional[float] = Field(None, description="Truth score (0-1)")
...
async def rag_translate(
    ...
    try:
        ...
        result = RagTranslationResult(
            ...
            # Add new metrics fields
            performance_metrics=translation_result.performance_metrics,
            memory_usage=translation_result.memory_usage,
            operation_cost=translation_result.operation_cost,
            accuracy_score=translation_result.accuracy_score,
            truth_score=translation_result.truth_score
        )
...
async def rag_query(
    ...
    try:
        ...
        query_response = RagQueryResponse(
            ...
            # Add new metrics fields
            performance_metrics=getattr(retriever, "performance_metrics", None),
            memory_usage=getattr(retriever, "memory_usage", None),
            operation_cost=getattr(retriever, "operation_cost", None),
            accuracy_score=getattr(retriever, "accuracy_score", None),
            truth_score=getattr(retriever, "truth_score", None)
        )
...
async def rag_chat(
    ...
    try:
        ...
        chat_response = RagChatResponse(
            ...
            process_time=time.time() - start_time,
            # Add new metrics fields
            performance_metrics=getattr(generator, "performance_metrics", None),
            memory_usage=getattr(generator, "memory_usage", None),
            operation_cost=getattr(generator, "operation_cost", None),
            accuracy_score=getattr(generator, "accuracy_score", None),
            truth_score=getattr(generator, "truth_score", None)
        )
