...
async def check_component_status(request: Request) -> List[ComponentStatus]:
    ...
    if hasattr(request.app.state, "metrics") and request.app.state.metrics:
        ...
        try:
            ...
            request_metrics = system_metrics.get("request_metrics", {})
            metrics_details = {
                "total_requests": request_metrics.get("total_requests", 0),
                "successful_requests": request_metrics.get("successful_requests", 0),
                "failed_requests": request_metrics.get("total_errors", 0),
                "avg_response_time": request_metrics.get("avg_response_time", 0.0),
                "uptime": system_metrics.get("uptime_seconds", 0.0)
            }
