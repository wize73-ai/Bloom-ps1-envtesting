...
class TranslationPipeline:
    ...
    async def detect_language(self, request: LanguageDetectionRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, dict) and "result" in result:
                ...
                # Log if metrics are available
                if any([performance_metrics, memory_usage, operation_cost, accuracy_score, truth_score]):
                    logger.debug("Enhanced metrics found in language detection result")
                else:
                    logger.debug("No enhanced metrics found in language detection result")
                ...
                if request.detailed:
                    ...
                    return {
                        ...
                        "detected_language": language,  # Add this field for compatibility
                        "confidence": confidence,
                        # Include enhanced metrics even for non-detailed request
                        "performance_metrics": performance_metrics,
                        "memory_usage": memory_usage,
                        "operation_cost": operation_cost,
                        "accuracy_score": accuracy_score,
    ...
    async def translate(self, request: TranslationRequest) -> TranslationResult:
        ...
        try:
            ...
            if isinstance(result, dict) and "result" in result:
                ...
                # Log if metrics are available
                if any([performance_metrics, memory_usage, operation_cost, accuracy_score, truth_score]):
                    logger.debug("Enhanced metrics found in translation result")
                else:
                    logger.debug("No enhanced metrics found in translation result")
                ...
                return TranslationResult(
                    ...
                    target_language=target_language,
                    confidence=0.0,
                    # Include null enhanced metrics for consistency
    ...
    async def translate_text(
        ...
        if model_id != "translation" and use_fallback and self._is_poor_quality_translation(result.translated_text, text):
            ...
            # Try fallback with MBART model (now configured as "translation")
            try:
                # Get MBART language codes
                mbart_source_lang = self._get_mbart_language_code(source_language)
                mbart_target_lang = self._get_mbart_language_code(target_language)
                ...
                fallback_request = TranslationRequest(
                    ...
                    source_language=source_language,
                    target_language=target_language,
                    model_name="translation",  # Use MBART model via "translation" key
                    glossary_id=glossary_id,
                    preserve_formatting=preserve_formatting,
                    formality=formality,
                    parameters={
        ...
        response = {
            ...
            "model_used": result.model_used if model_id != "translation" else "mbart",
