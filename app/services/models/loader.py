...
# Disable progress bars from HuggingFace to avoid colorama issues
...
try:
    ...
    from transformers import MT5ForConditionalGeneration, T5ForConditionalGeneration, MBartForConditionalGeneration
...
class ModelLoader:
    ...
    def load_model(self, model_type: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            # First check for transformers library before proceeding
    ...
    def _load_transformers_model(self, model_config: ModelConfig, device: str) -> Any:
        ...
        from transformers import (
            ...
            T5ForConditionalGeneration, MBartForConditionalGeneration,
        ...
        try:
            ...
            if hasattr(model_config, "model_class") and model_config.model_class:
                ...
                if model_config.model_class == "T5ForConditionalGeneration":
                    ...
                    # Try to use the specified class dynamically
                    try:
                        import transformers
            ...
            # Fall back to task-based selection
            elif model_config.task == "translation" or model_config.task == "rag_generation" or model_config.task == "mbart_translation":
                ...
                if "mbart" in model_config.model_name.lower() or model_config.task == "mbart_translation":
                    ...
                    # Force newer download parameters for MBART
                    model_kwargs["local_files_only"] = False
                    model_kwargs["force_download"] = True
                    
                    # Try with AutoModelForSeq2SeqLM first which is often more robust with broken caches
                    try:
                        # For MBART models, try using AutoModelForSeq2SeqLM first which can be more robust
                        logger.info(f"Trying to load MBART with AutoModelForSeq2SeqLM: {model_config.model_name}")
                        model = AutoModelForSeq2SeqLM.from_pretrained(
                            model_config.model_name,
                            **model_kwargs
                        )
                        logger.info(f"Successfully loaded MBART model using AutoModelForSeq2SeqLM")
                        return model
                    except Exception as e1:
                        logger.warning(f"Failed to load MBART with AutoModelForSeq2SeqLM: {e1}, trying direct implementation")
                        try:
                            # Try direct MBartForConditionalGeneration implementation
                            model = MBartForConditionalGeneration.from_pretrained(
                                model_config.model_name, 
                                **model_kwargs
                            )
                            logger.info(f"Successfully loaded MBART model using MBartForConditionalGeneration")
                            return model
                        except Exception as e2:
                            logger.warning(f"Failed to load as MBartForConditionalGeneration: {e2}, trying alternate model")
                            
                            # As a last resort, try an alternate model path
                            alternate_model = "facebook/mbart-large-50-one-to-many-mmt"
                            logger.info(f"Trying alternate MBART model: {alternate_model}")
                            try:
                                model = AutoModelForSeq2SeqLM.from_pretrained(
                                    alternate_model,
                                    **model_kwargs
                                )
                                logger.info(f"Successfully loaded alternate MBART model: {alternate_model}")
                                return model
                            except Exception as e3:
                                logger.error(f"All MBART model loading attempts failed: {e3}")
                                raise ValueError(f"Failed to load any MBART model variant")
                # Check if this is an MT5 model
                    ...
                    # Force newer download parameters for MT5
                    model_kwargs["local_files_only"] = False 
                    model_kwargs["force_download"] = True
                    
                    try:
                        # Try to load with AutoModelForSeq2SeqLM first for robustness
                        logger.info(f"Trying to load MT5 with AutoModelForSeq2SeqLM: {model_config.model_name}")
                        model = AutoModelForSeq2SeqLM.from_pretrained(
                            model_config.model_name,
                            **model_kwargs
                        )
                        ...
                        logger.warning(f"Failed to load MT5 with AutoModelForSeq2SeqLM: {e1}, trying direct implementation")
                        try:
                            # Try direct MT5ForConditionalGeneration implementation
                            ...
                            logger.warning(f"Failed to load as MT5ForConditionalGeneration: {e2}")
                            
                            # Try a smaller MT5 model as fallback
                            alternate_model = "google/mt5-small"
                            logger.info(f"Trying smaller MT5 model: {alternate_model}")
                            try:
                                model = AutoModelForSeq2SeqLM.from_pretrained(
                                    alternate_model,
                                    **model_kwargs
                                )
                                logger.info(f"Successfully loaded smaller MT5 model: {alternate_model}")
                                return model
                            except Exception as e3:
                                logger.error(f"All MT5 model loading attempts failed: {e3}")
                                raise ValueError(f"Failed to load any MT5 model variant")
                else:
                    # Use AutoModelForSeq2SeqLM for other translation models
                    model = AutoModelForSeq2SeqLM.from_pretrained(
                        model_config.model_name, 
                        **model_kwargs
                    )
    ...
    def _load_tokenizer(self, model_config: ModelConfig) -> Any:
        ...
        from transformers import AutoTokenizer, MBart50TokenizerFast, MBartTokenizer, T5Tokenizer, MT5Tokenizer
        ...
        if "mbart" in model_config.model_name.lower() or model_config.task == "mbart_translation":
            ...
            # Add critical parameters for MBART tokenizers
            tokenizer_kwargs["src_lang"] = tokenizer_kwargs.get("src_lang", "en_XX")
            tokenizer_kwargs["tgt_lang"] = tokenizer_kwargs.get("tgt_lang", "es_XX")
            
            # Enable local files use to avoid download issues
            ...
            if "50" in model_config.model_name or "many-to-many" in model_config.model_name:
                try:
                    # Use more robust path to the MBART tokenizer vocab
                    mbart_dir = "facebook/mbart-large-50-many-to-many-mmt"
                    # Try with the AutoTokenizer first which can be more robust
                    tokenizer = AutoTokenizer.from_pretrained(
                        mbart_dir,
                        use_fast=True,
                        **tokenizer_kwargs
                    )
                    logger.info(f"Successfully loaded MBART tokenizer using AutoTokenizer")
                    return tokenizer
                except Exception as e:
                    logger.warning(f"Failed to load MBART tokenizer via AutoTokenizer: {e}, trying direct implementation")
                    try:
                        ...
                        logger.warning(f"Failed to load MBart50TokenizerFast: {e}, falling back to AutoTokenizer")
            else:
                # For other MBART variants
                try:
                    # Try with the AutoTokenizer first which can be more robust
                    tokenizer = AutoTokenizer.from_pretrained(
                        model_config.tokenizer_name,
                        use_fast=False,  # Try non-fast version for better compatibility
                        **tokenizer_kwargs
                    )
                    logger.info(f"Successfully loaded MBART tokenizer using AutoTokenizer")
                    return tokenizer
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to load MBartTokenizer: {e}, falling back to AutoTokenizer")
        
        # Special handling for MT5 models
        elif "mt5" in model_config.model_name.lower() or model_config.task == "mt5_translation":
            logger.info(f"Loading specialized MT5 tokenizer for {model_config.tokenizer_name}")
            
            # Force download for MT5 tokenizer
            tokenizer_kwargs["local_files_only"] = False
            tokenizer_kwargs["force_download"] = True
            
            # First try AutoTokenizer which is most robust
            try:
                tokenizer = AutoTokenizer.from_pretrained(
                    model_config.tokenizer_name,
                    **tokenizer_kwargs
                )
                logger.info(f"Successfully loaded MT5 tokenizer using AutoTokenizer")
                return tokenizer
            except Exception as e:
                logger.warning(f"Failed to load tokenizer via AutoTokenizer: {e}, trying MT5Tokenizer")
                try:
                    # Use MT5Tokenizer directly
                    from transformers import MT5Tokenizer
                    return MT5Tokenizer.from_pretrained(
                        model_config.tokenizer_name, 
                        **tokenizer_kwargs
                    )
                except Exception as e:
                    logger.warning(f"Failed to load MT5Tokenizer: {e}, falling back to T5Tokenizer")
                    try:
                        # Fall back to T5Tokenizer
                        ...
                        logger.warning(f"Failed to load T5Tokenizer: {e}, falling back to AutoTokenizer")
        
        # Default tokenizer loading using AutoTokenizer
    ...
    def _load_sentence_transformer(self, model_config: ModelConfig, device: str) -> Any:
        ...
        """
        # Check for the sentence-transformers library
        if not HAVE_SENTENCE_TRANSFORMERS:
            raise ImportError("sentence_transformers library is required for embedding models")
        
        # Import the SentenceTransformer class directly
        from sentence_transformers import SentenceTransformer
