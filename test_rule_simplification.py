#!/usr/bin/env python3
"""
Test script for rule-based text simplification in CasaLingua.

This script directly tests the rule-based simplification functionality 
with different simplification levels.
"""
...
def rule_based_simplify(text: str, level: int) -> str:
...
def test_rule_simplification():
    """Test rule-based simplification with different levels."""
    print(f"\n{BOLD}{BLUE}Testing Rule-Based Simplification Levels{ENDC}")
    print("-" * 80)
    ...
    original_metrics = calculate_metrics(TEST_TEXT)
    print(f"{BOLD}Original Grade Level:{ENDC} {original_metrics['grade_level']}")
    print(f"{BOLD}Words per Sentence:{ENDC} {original_metrics['words_per_sentence']}")
    print(f"{BOLD}Words:{ENDC} {original_metrics['words']}")
    print("-" * 80)
    ...
    for level in range(1, 6):
        ...
        simplified = rule_based_simplify(TEST_TEXT, level)
        ...
        print(f"{BOLD}Words per Sentence:{ENDC} {metrics['words_per_sentence']}")
        print(f"{BOLD}Words:{ENDC} {metrics['words']}")
        
        # Calculate text diff percentage
        diff_percent = (len(simplified) - len(TEST_TEXT)) / len(TEST_TEXT) * 100
        diff_color = GREEN if diff_percent < 0 else YELLOW if diff_percent > 0 else BLUE
        print(f"{BOLD}Change:{ENDC} {diff_color}{diff_percent:.1f}%{ENDC}")
